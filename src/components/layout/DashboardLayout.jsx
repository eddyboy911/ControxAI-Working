...
import { currencies } from '../dashboard/Settings';
...
const DashboardLayout = () => {
    ...
    const navItems = [
        { icon: PieChart, label: 'Overview', path: '/dashboard/overview' },
        { icon: Phone, label: 'Call Logs', path: '/dashboard/call-logs' },
        { icon: Calendar, label: 'Bookings', path: '/dashboard/bookings' },
        { icon: Zap, label: 'Test Agent', path: '/dashboard/test-agent' },
        { icon: Clock, label: 'Analytics', path: '/dashboard/analytics' },
        { icon: DollarSign, label: 'Billing', path: '/dashboard/billing' },
        { icon: SettingsIcon, label: 'Settings', path: '/dashboard/settings' },
    ];

    const currentTab = navItems.find(item => location.pathname.includes(item.path)) || navItems[0];
    const activeTabLabel = currentTab.label;
                ...
                >
                    <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pt-0 md:pt-4">
                        <div className="flex-1">
                            {activeTabLabel === 'Overview' ? (
                                ...
                                <div>
                                    <h1 className="text-3xl font-bold text-white tracking-tight">{activeTabLabel}</h1>
                                    <p className="text-sm text-gray-400 mt-1 font-medium">
                                        {tabInfo[activeTabLabel]?.subtext}
                                    </p>
                                </div>
                            )}
                        </div>
                    ...
                    {loading ? (
                        ...
                        <Outlet context={{
                            ...
                            stats, activityData, pieData, recentCalls,
                            overviewFilter, setOverviewFilter,
                            formattedAllCalls, loading,
                            bookings, allCalls, bookingsLoading,
                            organization
                        }} />
                    )}
                </motion.div>
